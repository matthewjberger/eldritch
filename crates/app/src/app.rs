					...
					if is_swapchain_dirty {
						...
						if dim.width > 0 && dim.height > 0 {
							app.recreate_swapchain()
								.expect("Failed to recreate swap");
						} else {
							return;
						}
					}
...
impl Triangle {
	fn new(window: &Window) -> Result<Self, Box<dyn Error>> {
		...
		// Pipeline and layout
		let (pipeline, pipeline_layout) = create_vulkan_pipeline(
			&device,
			swapchain_extent,
			swapchain_format,
		)?;
...
) -> Result<(Instance, DebugUtils, vk::DebugUtilsMessengerEXT), Box<dyn Error>>
{
	...
	let (major, minor) = match entry.try_enumerate_instance_version()? {
		// Vulkan 1.1+
		Some(version) => {
			(vk::api_version_major(version), vk::api_version_minor(version))
		}
		// Vulkan 1.0
		None => (1, 0),
	};
...
fn create_vulkan_pipeline(
	...
	let vertex_source = read_shader_from_bytes(
		&include_bytes!("../../../assets/shaders/shader.vert.spv")[..],
	)?;
	let vertex_create_info =
		vk::ShaderModuleCreateInfo::builder().code(&vertex_source);
	let vertex_module =
		unsafe { device.create_shader_module(&vertex_create_info, None)? };

	let fragment_source = read_shader_from_bytes(
		&include_bytes!("../../../assets/shaders/shader.frag.spv")[..],
	)?;
	let fragment_create_info =
		vk::ShaderModuleCreateInfo::builder().code(&fragment_source);
	let fragment_module =
		unsafe { device.create_shader_module(&fragment_create_info, None)? };
...
fn create_and_record_command_buffers(
	...
	for (index, buffer) in buffers.iter().enumerate() {
		...
		unsafe {
			device.begin_command_buffer(buffer, &command_buffer_begin_info)?
		};
